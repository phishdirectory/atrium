...
class EmailsController < ApplicationController
  ...
  def index
    ...
    @emails = case @folder
              when "inbox"
                current_profile.emails.where.not(received_at: nil).where(archived: false).order(received_at: :desc).includes(:rich_text_body)
              when "sent"
                current_profile.emails.where.not(sent_at: nil).order(sent_at: :desc).includes(:rich_text_body)
              when "starred"
                current_profile.emails.where(starred: true).order(created_at: :desc).includes(:rich_text_body)
              when "archived"
                current_profile.emails.where(archived: true).order(created_at: :desc).includes(:rich_text_body)
              else
                current_profile.emails.where.not(received_at: nil).where(archived: false).order(received_at: :desc).includes(:rich_text_body)
              end
    ...
    if params[:id].present?
      @email = current_profile.emails.find_by(id: params[:id])
    end
    ...
    if @email&.message_id.present?
      @conversation = current_profile.emails
  ...
  def new
    ...
    original = current_profile.emails.find(params[:reply_to])
  ...
  def create
    @email = current_profile.mailbox.emails.new(email_params)
    @email.sender = current_profile.mailbox.email_address
